...
export default class SendWidgetController {
  constructor($scope, $rootScope, $sce, Sessions, Server, Alerts) {
    ...
    this.memo = false;
    this.memoType = null;
    this.nezAsset = new Asset('ZEN', 'GAVWNTZRZGIWQDUDOWLMBU52LYYEVLDKWZVW3PBUIJXWF4NZREFXMJBR');
    this.assetType = null;
    this.memoValue = null;
    ...
    this.assetAlertGroup = new AlertGroup();
    this.assetAlertGroup.registerUpdateListener(alerts => {
      // Some amount alert messages contain HTML
      for (let alert of alerts) {
        alert._text = $sce.trustAsHtml(alert._text)
      }
      this.assetAlerts = alerts;
    });
    Alerts.registerGroup(this.assetAlertGroup);
  ...
  send() {
    ...
    if (!Operation.isValidAmount(this.amount)) {
      ...
      this.amountAlertGroup.show(alert);
        ...
        // Check if amount is valid
    if ((this.assetType != "NEZ") && (this.assetType != "LUMENS")) {
      let alert = new Alert({
        title: '',
        text: 'Please Select Asset',
        type: Alert.TYPES.ERROR
      });
      this.assetAlertGroup.show(alert);
      this.sending = false;
      return;
    }
    ...
    return this.Server.accounts()
      ...
      .then(account => {
        ...
        let nezBalance = _(account.balances).find(balance => balance.asset_code === 'ZEN').balance;
        
        console.log("NEZ BALANCE:"+ nezBalance + " this.amount->"+this.amount);

        let maxSend = new BigNumber(nativeBalance).minus(minimumBalance);
        let nezMaxSend = new BigNumber(nezBalance);

        if(this.assetType=="NEZ"){

          if (nezMaxSend.lt(this.amount)) {
            throw new BasicClientError('InsufficientBalanceError', {nezMaxSend});
          }
