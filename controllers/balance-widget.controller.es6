...
let nezbalancedisplay;
    if (_.isArray(balances) && balances.length > 0) {

        
  let nezBalance = _(balances).find(balance => balance.asset_code === 'ZEN');

  //console.log("Balance NEZ : "+ JSON.stringify(balances) );
  
  //console.log("Balance NEZ : "+ nezBalance.balance );

      if (nezBalance) {
        nezbalancedisplay = nezBalance.balance;
      } else {
        nezbalancedisplay = 0;
      }
    } else {
    ...
    this.nezbalancedisplay = new BigNumber(nezbalancedisplay).toFormat();
    this.nezbalanceLoaded = true;
